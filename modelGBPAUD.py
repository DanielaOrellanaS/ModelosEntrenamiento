"""
Modelo v4 GBPAUD — idéntico a NAS100
Solo datos 5m: OHLCV + RSI + Stoch + EMA/MACD/ADX
Profit convertido a pips (×10000) antes de normalizar
"""
...
def evaluate_model(model, dataloader, min_profit, max_profit, label):
    ...
    print(f"\n── {label.upper()} ─────────────────────────────────")
    print(f"  Clasificación   → Accuracy: {accuracy*100:.1f}%  |  MAE profit: {mae:.2f} pips")
    print(f"  Alta confianza  → {mask.sum()}/{len(conf)} ops ({mask.sum()/len(conf)*100:.0f}%)  |  Umbral: {threshold:.3f}")
    if mask.sum():
        print(f"  Señales         → BUY: {buys} ({buys/mask.sum()*100:.1f}%)  SELL: {sells} ({sells/mask.sum()*100:.1f}%)")
    else:
        print("  Sin señales")
    print(f"  Acc filtrado    → {filt_acc*100:.1f}%")
    print(f"  Trading         → Profit: {total_prof:.2f} pips  |  Win rate: {win_rate:.1f}%  |  PF: {pf:.2f}")
...
def main():
    ...
    data = pd.read_excel(os.path.join(DATA_DIR, "Data_Entrenamiento_GBPAUD.xlsx"))
    ...
    for ds in [train_data, val_data, test_data]:
        ...
        ds['profit_original'] = ds['profit'].copy()
    ...
    best_path        = os.path.join(MODEL_DIR, "best_trading_model_GBPAUD_v4.pth")
    ...
    print(f"\n── BRECHA VAL vs TEST ──────────────────────────────")
    print(f"  Accuracy gap: {acc_gap:.1f}%  |  MAE gap: {mae_gap:.2f} pips  →  {status}")

    torch.save(model.state_dict(), os.path.join(MODEL_DIR, "trading_model_GBPAUD_v4.pth"))
    scaler = {
        ...
        "pips_factor": PIPS_FACTOR,
    }
    scaler.update(scaler_extra)
    with open(os.path.join(MODEL_DIR, "scaler_GBPAUD_v4.pkl"),        "wb") as f: pickle.dump(scaler,        f)
    with open(os.path.join(MODEL_DIR, "input_columns_GBPAUD_v4.pkl"), "wb") as f: pickle.dump(input_columns, f)
    print(f"\n✅ Modelo GBPAUD v4 guardado en {MODEL_DIR}")
